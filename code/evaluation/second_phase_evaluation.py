...
def evaluation_with_classifiers(AUT, usage_name):

    start = time.time()
    test_gen = TestGenerator(AUT.desiredCapabilities, REMAUI_flag=True, eval_flag=True, use_TRUE_label_flag=False)
    ...
    output_path = os.path.join(final_data_root, 'output', 'models', usage_name, 'dynamic_output_phase_2')
...
if __name__ == '__main__':
    AUT = Reuters()
    usage_name = usage_folder_map['removebookmark'] # this usage_folder_map is defined in global_config.py
