...
4. double check (2) final labels and (3) IR definitions to make sure it's consistent, e.g., all the labels in the final_labels should be in our definitions, all screen labels should NOT be empty (since no cropping error for screens, but some widget labels can be empty which are labeled as N/A)
